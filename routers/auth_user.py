...
def register_tenant_admin(
    ...
    if not invite_payload:
        ...
        if invite_status == "revoked":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="系统管理员邀请码已作废")
        if invite_status == "expired":
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="系统管理员邀请码已过期")
        if invite_record:
            raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="系统管理员邀请码无效")
